...
- 날짜: 2026-08-15
- 상태: 승인됨 — **1단계(배관) 구현 완료 (2026-08-15, 브랜치 `feature/live-smile-monitor`, `d6f638f`…)**. 자동 검증 전부 통과: CoachingKit 301 tests, 앱 테스트, 카탈로그 게이트 181키/`missing_lang=0`/`needs_review=183`, CoachingKit 사용자 문자열 0(마이그레이션 스냅샷 8줄 제외), 한국어 보존 diff(placeholder 치환 외 차이 없음). 시뮬레이터: 한국어 기록 화면 정상, 영어 기기에서 영어 UI·"SmileDay" 아이콘. **실기기/탭이 필요해 남긴 것**: 영국 지역 요일 헤더 7열, 로컬 알림 언어 전환(push 경로로는 통과), v1→v2 업그레이드 경로, `.accessibility5`+SE. 계획 Task 14 Step 3–6 참조. **2단계(영어 집필) 완료 (2026-08-16)** — 181키 전부 translated, `STRICT=1` 게이트 통과, 영어/한국어 시뮬레이터 양방향 확인. 스토어 영어 메타데이터는 `docs/marketing/2026-08-16-app-store-metadata-en.md`. 영어 스크린샷 4장(6.9인치)은 `docs/marketing/screenshots/app-store-6.9-en/`, 정책·지원 영어 본문은 `docs/marketing/2026-08-16-privacy-support-en.md`. 시뮬레이터 추가 검증: 영국 지역 달력 M-시작 7열 정상, 최대 접근성 크기에서 줄바꿈(잘림 없음). **App Store Connect 입력 완료 (2026-08-16)**: 버전 1.1 생성, 영어(미국) 현지화 추가 — 이름 SmileDay(중복 없이 저장됨)·부제·프로모션·설명·키워드·What's New(영/한)·6.5" 스크린샷 4장(1284×2778, `app-store-6.5-en/`) 입력·저장. 심사 제출은 하지 않음. **1.1 심사 제출 완료 (2026-08-16)**: 버전 1.1(빌드 2)을 CLI로 아카이브·업로드하고 (`MARKETING_VERSION 1.1`/`CURRENT_PROJECT_VERSION 2`), ASC에서 빌드 첨부·저장 후 심사에 제출했다. 승인되면 모든 사용자에게 자동 즉시 출시된다(최대 48시간, 완료 시 이메일). **남은 것**: dolparo.com 두 페이지에 영어 섹션 게시, Primary Language 전환 — **시도했으나 ASC가 거부(2026-08-16)**: "해당 언어로 된 필수 스크린샷을 각 버전별로 모두 제공해야" 하는데 1.0(배포 준비됨)은 잠겨 있어 영어 현지화를 추가할 수 없다. **1.1이 출시되어 1.0을 대체한 뒤 다시 전환한다** (앱 정보 → 기본 언어 → 영어(미국) → 저장), 실기기 확인 2건(로컬 알림 언어 전환·v1→v2 업그레이드).
