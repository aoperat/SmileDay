...
| File | Description |
|------|-------------|
| `SmileDayApp.swift` | `@main` App — wires AppDelegate via adaptor, builds the SwiftData `ModelContainer` from `PersistenceSchema`, hosts `RootView` with `ko_KR` locale, injects `NotificationRouter` into the environment |
