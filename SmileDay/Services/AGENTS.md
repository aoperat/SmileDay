...
- `frame.capturedImage` is read in exactly one place: `snapshotJPEGData()`, called at most once a minute to produce downscaled JPEG bytes for the session summary. It is not read per frame, and there is no path that writes it to disk. It returns `Data` rather than a `UIImage` on purpose — an uncompressed 240×320 image costs ~300KB, so holding the session's photos as pixels would run about twelve times the memory budget in the spec.
- It exposes `previewSession` so the optional camera view can draw the session that is already running, and `reassertSampleDelegate()` because `ARSCNView` may claim `session.delegate` and silently kill the sample stream. Neither is a hook for saving frames — the app target still owns the snapshot array and discards it when the summary closes.
