...
- `frame.capturedImage` is read in exactly one place: `snapshotImage()`, called at most once a minute to produce a downscaled in-memory image for the session summary. It is not read per frame, and there is no path that writes it to disk.
