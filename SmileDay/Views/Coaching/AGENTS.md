...
- `ARFrame.capturedImage` is converted only for the once-a-minute snapshot, via `ARKitLiveSmileMonitor.snapshotJPEGData()`, which returns JPEG `Data`, not `UIImage` — the raw `CGImage` is never held past that call. Never convert per frame, and never add a capture, save, export, or share button.
- `LiveSmileMonitorView` owns the snapshot array (`[Data]` of JPEG bytes) and passes it down to the summary screen as a `let`. Closing the monitor screen must clear both the array and the summary — that is the only thing standing between "session only" and "stored".
