...
**Goal:** 실시간 확인을 끝냈을 때 그 세션의 타임라인 그래프와 미소 비율, 분당 1장 스냅샷을 보여주고, 화면을 닫으면 전부 메모리에서 버린다.

**Architecture:** 초 단위 집계는 `CoachingKit`의 `LiveSmileSessionRecorder`가 순수 로직으로 담당한다. 프레임마다 `observe`를 받아 1초 칸을 다수결로 확정하고, 프레임이 끊긴 칸은 `unknown`으로 채운다. 패키지는 UIKit을 import하지 않으므로 이미지는 앱 타깃이 들고, 스냅샷 타이밍만 패키지가 알려준다.

**Tech Stack:** Swift 5.10, SwiftUI, ARKit, CoreImage, Observation, XCTest, iOS 17+.
...
- [ ] 요약을 닫고 다시 들어가면 이전 세션의 띠와 사진이 없다
...
결과는 `SmileDay/docs/reports/YYYY-MM-DD-live-smile-session-graph-device-verification.md`에 기기, iOS, 빌드, 항목별 PASS/FAIL과 발열 체감만 기록한다. **얼굴 사진, raw blend shape, 비율 시계열은 저장하지 않는다.**
